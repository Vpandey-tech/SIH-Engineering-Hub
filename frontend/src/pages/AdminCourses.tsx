...
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Trash2, Edit, Plus, Eye, BookOpen, Users, TrendingUp, Sparkles } from "lucide-react";
import { toast } from "sonner";
import API from "../services/api";
...
const AdminCourses: React.FC = () => {
  ...
  const handleGenerateQuiz = async (lectureId: string) => {
    setGeneratingQuizId(lectureId);
    try {
      const res = await API.post(`/lms/lectures/${lectureId}/generate-quiz`);
                        ...
                        {lecturesMap[course.id].map(l => (
                          ...
                          >
                            ...
                            <div className="flex gap-1 ml-2">
                              <Button
                                size="sm"
                                className="h-7 px-2 bg-teal-500 hover:bg-teal-600 text-white text-xs"
                                onClick={() => handleGenerateQuiz(l.id)}
                                disabled={generatingQuizId === l.id}
                              >
                                {generatingQuizId === l.id ? (
